...
def run(vars,stage):
    if (stage == "training"):
      path_plots = loc.PLOTS
    elif (stage == "validation"):
      path_plots = loc.PLOTS_Val
    else:
      print("stage unknown")
      exit(1)

    print("Writing output to output file")
    ut.create_dir(path_plots)
    print(path_plots)
    # Load trained model
    ...
    #plt.grid()
    #plt.tight_layout()
    #fig.savefig(f"{path_plots}/mumuH_vs_ZZ_WWmumu_Zll_eeZ_ROC_{vars}.pdf")
    ...
    fig.savefig(f"{path_plots}/mumuH_vs_ZZ_WWmumu_Zll_eeZ_Significance_Z0_{vars}.pdf") 
    ...
    fig.savefig(f"{path_plots}/mumuH_vs_ZZ_WWmumu_Zll_eeZ_Significance_Z_{vars}.pdf") 
    ...
    plt.tight_layout()
    fig.savefig(f"{path_plots}/mumuH_vs_ZZ_WWmumu_Zll_eeZ_BDT_{vars}.pdf")
    ...
    plt.tight_layout()
    fig.savefig(f"{path_plots}/mumuH_vs_ZZ_WWmumu_Zll_eeZ_BDT_eff_nolog_{vars}.pdf")
    ...
    plt.grid(alpha=0.4,which="both")
    plt.tight_layout()
    fig.savefig(f"{path_plots}/mumuH_vs_ZZ_WWmumu_Zll_eeZ_BDT_Sig_{vars}.pdf")
...
def main():
    ...
    parser.add_argument("--Stage", choices=["training","validation"],required=True,help="training or validation")
    args = parser.parse_args()

    run(args.Vars,args.Stage)
