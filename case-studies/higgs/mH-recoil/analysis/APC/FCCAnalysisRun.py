...
def sendToBatch(rdfModule, chunkList, process, analysisFile):
    ...
    for ch in range(len(chunkList)):
        ...
        frun.write('source /cvmfs/sw.hsf.org/spackages6/key4hep-stack/2023-01-15/x86_64-centos7-gcc11.2.0-opt/csapx/setup.sh\n')
        ...
        if not os.path.isabs(outputDir):
            frun.write('python {}/FCCAnalysisRun.py {} --batch --output {}chunk{}.root --files-list '.format(programDir, analysisFile, outputDir, ch))
        else:
            frun.write('python {}/FCCAnalysisRun.py {} --batch --output {}{}/chunk{}.root --files-list '.format(programDir, analysisFile, outputDir, process,ch))
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(analysisFile):
        ...
        print("python analysis/APC/FCCAnalysisRun.py analysis.py <options>")
        sys.exit(3)
