...
def Count(Histos,Variable,Label,Selection):
  ...
  print("")
# ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
def Significance_binbybin(Histos,Variable,Label,Selection,path):
  #print(len(histos))
  Significance = {}
  nbins = Histos['signal']["mumuH"][0].GetXaxis().GetNbins()
  X_min = Histos['signal']["mumuH"][0].GetXaxis().GetXmin()
  X_max = Histos['signal']["mumuH"][0].GetXaxis().GetXmax() 
  bin_width = Histos['signal']["mumuH"][0].GetXaxis().GetBinWidth(0)
  h_significance = ROOT.TH1F("Significance", "Significance", nbins, X_min, X_max)
  for i in range(nbins):
    Si = 0
    Bi = 0
    for channel in Histos['signal']:
      Si = Histos["signal"][channel][0].GetBinContent(i)
    for channel in Histos['backgrounds']:
      Bi += Histos["backgrounds"][channel][0].GetBinContent(i) 
    Significance[i] = ut.Z(Si,Bi)
    print(f"S:{Si}  B:{Bi}  Significance:{Significance[i]}")
    h_significance.SetBinContent(i,Significance[i])
  c = ROOT.TCanvas("c")
  h_significance.Draw("histo")
  #c.Draw()
  c.SaveAs(f"{path}/Significance_{Label}_{Selection}_{Variable}.pdf")
  #print("------------------------------------------------------------------------------------ ")
  #print('{:<40s} {:>10.1f} {:>10.1f} {:>10.0f}'.format('Total_'+keygroup,yield_total,Error_total,Entries_total))
  #print("------------------------------------------------------------------------------------ ")
# ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
...
if __name__ == "__main__":
  ...
  outDir = "/eos/user/l/lia/FCCee/MVA/trainedNtuples/plots_significance"
  ...
  for label, process in processes.items():
    for sel in selections:
      ...
      Significance_binbybin(histo,var,label,sel,outDir)
